...
if __name__ == "__main__":
    ...
    #모델을 만들고 훈련시킨다
    conv_autocoder = Sequential()
    conv_autocoder.add(Conv2D(filters=16, kernel_size=(3,3), activation='relu', padding='same', input_shape=(28,28,1)))
    conv_autocoder.add(Conv2D(filters=8, kernel_size=(3,3), activation='relu', padding='same'))
    conv_autocoder.add(Conv2D(filters=8, kernel_size=(3,3), activation='relu', padding='same'))
    conv_autocoder.add(Conv2D(filters=16, kernel_size=(3,3), activation='relu', padding='same'))
    conv_autocoder.add(Conv2D(filters=1, kernel_size=(3,3), activation='sigmoid', padding='same'))
    conv_autocoder.summary()
    conv_autocoder.compile(optimizer='adam', loss='binary_crossentropy')
    conv_autocoder.fit(X_train_noisy.reshape(60000,28,28,1), X_train_reshaped.reshape(60000,28,28,1), epochs=10)

    output = conv_autocoder.predict(X_test_noisy.reshape(10000,28,28,1))
    # 결과 출력
